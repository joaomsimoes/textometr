...
class TestAnalyzer(unittest.TestCase):
    ...
    def test_text_26(self):
        self.assertEqual(
            TestAnalyzer.text_analyzer.start_native('Из детских стихов всем известно, что в Африке всегда жарко, '
                                                    ...
                                                    'ярких бабочек, шумных попугаев, юрких обезьянок и других животных. ')['level_comment'],
            '19 баллов из 100. Очень простой текст, подойдет для возраста 7-8 лет (1-2 класс).'
        )

    def test_text_27(self):
        self.assertEqual(
            TestAnalyzer.text_analyzer.start_native('Когда произошла эта история, Сева жил у бабушки в деревне. '
                                                    ...
                                                    'шевелила головой.')['level_comment'],
            '24 балла из 100. Простой текст, подойдет для возраста 9-10 лет (3-4 класс).'
        )

    def test_text_28(self):
        self.assertEqual(
            TestAnalyzer.text_analyzer.start_native('Мистер и миссис Дурсль проживали в доме номер четыре по Тисовой '
                                                    ...
                                                    ' он был самым чудесным ребенком на свете.')['level_comment'],
            '40 баллов из 100. Простой текст, подойдет для возраста 9-10 лет (3-4 класс).'
        )

    def test_text_29(self):
        self.assertEqual(
            TestAnalyzer.text_analyzer.start_native('Почему перелетные птицы возвращаются весной обратно? '
                                                    ...
                                                    'которым они лучше всего приспособлены.')['level_comment'],
            '49 баллов из 100. Достаточно простой текст, подойдет для возраста 11-12 лет (5-6 класс).'
        )

    def test_text_30(self):
        self.assertEqual(
            TestAnalyzer.text_analyzer.start_native('Палеография – специальная наука, которая исследует памятники древней письменности.'
                                                    ...
                                                    'подготовленной мягкой поверхности.')['level_comment'],
            '75 баллов из 100. Текст подойдет для возраста 13-15 лет (7-9 класс).'
        )

    def test_text_31(self):
        self.assertEqual(
            TestAnalyzer.text_analyzer.start_native('«Министерство дурацких походок» (также распространен перевод '
                                                    ...
                                                    'порой тратят деньги на ерунду.')['level_comment'],
            '76 баллов из 100. Текст подойдет для возраста 16-17 лет (10-11 класс).'
        )

    def test_text_32(self):
        self.assertEqual(
            TestAnalyzer.text_analyzer.start_native('Договор аренды квартиры — документ подтверждающий предоставление '
